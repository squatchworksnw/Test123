...
const InteractionService = window.FieldOps.Services.interactions || {
  hydrateFromCache(){},
  seedDemoDataIfEmpty(){ return false; },
  localInsert(){ throw new Error("Local interaction service unavailable"); },
  localUpdate(){ throw new Error("Local interaction service unavailable"); },
  localArchive(){ throw new Error("Local interaction service unavailable"); },
  localRestore(){ throw new Error("Local interaction service unavailable"); },
  clearLocalPilotData(){},
  showToast(){},
  init(){}
};
...
function authContext(){ return { supabaseClient, app, getState:authState, setSession:value => { currentSession = value; }, setWorkspace:value => { currentWorkspace = value; }, createEmptyAppState:AppState.createEmptyAppState, titleize, setStatus, render, applyRoleVisibility, loadWorkspaceData, flushPendingWrites, startAutoRefresh }; }
...
function isLocalPilotMode(){ return !isAuthenticated(); }
function canManageOperations(){ return isLocalPilotMode() || AuthService.canManageOperations(authState()); }
...
function canAccessView(id){ return isLocalPilotMode() || AuthService.canAccessView(id, authState()); }
...
function requireInsertPermission(table, actionLabel = "create records"){ if(isLocalPilotMode()) return true; return AuthService.requireInsertPermission(table, actionLabel, authContext()); }
function requireUpdatePermission(table, actionLabel = "change records"){ if(isLocalPilotMode()) return true; return AuthService.requireUpdatePermission(table, actionLabel, authContext()); }
function requireArchivePermission(actionLabel = "archive records"){ if(isLocalPilotMode()) return true; return AuthService.requireArchivePermission(actionLabel, authContext()); }
function requireOperationsPermission(actionLabel = "manage operations"){ if(isLocalPilotMode()) return true; return AuthService.requireOperationsPermission(actionLabel, authContext()); }
...
async function signOutForSync(){ return AuthService.signOutForSync(authContext()); }
...
function hydrateLocalPilotState(){
  if(!isLocalPilotMode()) return;
  InteractionService.hydrateFromCache(app, Mappers);
}

function setLocalPilotStatus(message = "Saved locally — pilot mode", toneValue = "saved"){
  setStatus(message);
  InteractionService.showToast(message, toneValue);
}

function loadDemoPilotData(){
  InteractionService.seedDemoDataIfEmpty(app, Mappers, { force:true });
  setLocalPilotStatus("Demo data loaded locally", "saved");
  render();
}

function clearDemoPilotData(){
  if(!confirm("Clear local demo/pilot data from this browser?")) return;
  InteractionService.clearLocalPilotData(app);
  setLocalPilotStatus("Local demo data cleared", "saved");
  render();
}

globalThis.loadDemoPilotData = loadDemoPilotData;
globalThis.clearDemoPilotData = clearDemoPilotData;

async function insertRecord(table, payload){
  if(isLocalPilotMode()){
    const row = InteractionService.localInsert(table, payload, app, Mappers);
    setLocalPilotStatus("Saved locally — pilot mode", "saved");
    render();
    return row;
  }
...
async function updateRecord(table, idValue, payload){
  if(isLocalPilotMode()){
    const row = InteractionService.localUpdate(table, idValue, payload, app, Mappers);
    setLocalPilotStatus("Updated locally — pilot mode", "saved");
    render();
    return row;
  }
...
async function archiveRecord(table, idValue){
  if(isLocalPilotMode()){
    const row = InteractionService.localArchive(table, idValue, app, Mappers);
    setLocalPilotStatus("Archived locally — pilot mode", "saved");
    render();
    return row;
  }
...
async function restoreRecord(table, idValue){
  if(isLocalPilotMode()){
    const row = InteractionService.localRestore(table, idValue, app, Mappers);
    setLocalPilotStatus("Restored locally — pilot mode", "saved");
    render();
    return row;
  }
...
async function addTask(e){ e.preventDefault(); if(!requireOperationsPermission("create work orders")) return; try{ await insertRecord("field_ops_work_orders", Mappers.workOrderPayloadFromForm({ name:taskName.value, frequency:taskFrequency.value, status:taskStatus.value, priority:taskPriority.value, date:taskDate.value, projectId:taskProject.value, buildingId:taskBuilding.value, spaceId:taskSpace.value, assetId:taskAsset.value, vehicleId:taskVehicle.value, vendorId:taskVendorBid.value, location:taskLocation.value, notes:taskNotes.value })); e.target.reset(); }catch(err){ handleWriteError(err); } }
...
function applyRoleVisibility(){
  ...
  document.querySelectorAll(".tab").forEach(tab => {
    ...
    if(viewId === "importReview"){
      tab.textContent = canSubmitOnly() ? "Submit Request" : "Import Review";
    }
    if(viewId === "documents"){
      tab.textContent = canSubmitOnly() ? "Uploads" : "Documents";
    }
  });

  if(isAuthenticated() && currentWorkspace && !canAccessView(activeViewId)){
...
function resetLocalData(){ InteractionService.clearLocalPilotData(app); AppState.resetRuntimeApp(runtimeState); render(); setLocalPilotStatus("Temporary local cache cleared", "saved"); }
